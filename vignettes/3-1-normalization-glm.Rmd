...
h6.dat <- WellData(WellLocation("J107-2C", "H", 6))
h6.cln <- cleanData(h6.dat, "lower")
h6.mlt <- meltData(h6.cln)
h2.dat <- WellData(WellLocation("J107-2C", "H", 2))
h2.cln <- cleanData(h2.dat, "lower")
h2.mlt <- meltData(h2.cln)

prep <- prepareDataforGlm(h6.mlt$mat$Cells, h2.mlt$mat$Cells, test=NULL)
data.mint <- prep$train

```

Two wells, *MTOR* and *SCRAMBLED* or plate `r getBarcode(h6.dat)` are loaded, cleaned up, melted and prepared for glm fitting.
...
drop <- analyzeSeparation(data.mint, max.dim=1, all.dim=FALSE)
...
if(length(drop.names) > 0) {
  ...
  data.drop <- data.mint[, !names(data.mint) %in% drop.names]
}
...
ind.intens <- grepl("intensity", names(data.drop), ignore.case=TRUE)
if(length(ind.intens) > 0) {
  message("scale to unit interval (", length(ind.intens), " features):")
  l_ply(names(data.drop[ind.intens]), function(x) message("  ", x))
  feat.scal <- apply(data.drop[,ind.intens], 2, function(x) {
    return((x - min(x, na.rm=TRUE)) / diff(range(x, na.rm=TRUE)))
  })
  data.scal <- cbind(data.drop[,!ind.intens], data.frame(feat.scal))
  data.scal <- data.scal[,order(names(data.scal))]
}
...
if(length(ind.intens) > 0) {
  message("scale to ranges (", length(ind.intens), " features):")
  feat.rnge <- apply(data.drop[,ind.intens], 2, function(x) {
    return(x / diff(range(x, na.rm=TRUE)))
  })
  data.rnge <- cbind(data.drop[,!ind.intens], data.frame(feat.rnge))
  data.rnge <- data.rnge[,order(names(data.rnge))]
}
...
if(length(ind.intens) > 0) {
  message("center (", length(ind.intens), " features):")
  feat.cntr <- apply(data.drop[,ind.intens], 2, function(x) {
    return(x - mean(x))
  })
  data.cntr <- cbind(data.drop[,!ind.intens], data.frame(feat.cntr))
  data.cntr <- data.cntr[,order(names(data.cntr))]
}
...
if(length(ind.intens) > 0) {
  message("normalize variance (", length(ind.intens), " features):")
  feat.uvar <- apply(data.drop[,ind.intens], 2, function(x) {
    return(x / sd(x))
  })
  data.uvar <- cbind(data.drop[,!ind.intens], data.frame(feat.uvar))
  data.uvar <- data.uvar[,order(names(data.uvar))]
}
...
if(length(ind.intens) > 0) {
  message("z-score (", length(ind.intens), " features):")
  feat.zsco <- apply(data.drop[,ind.intens], 2, function(x) {
    return((x - mean(x)) / sd(x))
  })
  data.zsco <- cbind(data.drop[,!ind.intens], data.frame(feat.zsco))
  data.zsco <- data.zsco[,order(names(data.zsco))]
}
