---
title: "2.5 Stability analysis using glmnet"
...
vignette: >
  %\VignetteIndexEntry{2.5 Stability analysis using glmnet}
...
data1 <- unlist(getSingleCellData(c(mtor, scra)), recursive=FALSE)
data1 <- lapply(data1, cleanData, "lower")
data1 <- makeFeatureCompatible(data1)
reps  <- 100
...
mtor.h2.si <- lapply(lapply(mtor, getBarcode), function(bc, data1, n) {
  glmBootstrapStability(data1[[paste0(bc, ".H2")]], data1[[paste0(bc, ".H6")]],
                        n.rep=n, normalize=FALSE, alpha=0.5)
}, data1, reps)

mtor.h2.k1 <- glmBootstrapStability(data1[grep("-2C.H2$", names(data1))],
                                    data1[grep("-2C.H6$", names(data1))],
                                    n.rep=reps, normalize=FALSE, alpha=0.5)
mtor.h2.k2 <- glmBootstrapStability(data1[grep("-2D.H2$", names(data1))],
                                    data1[grep("-2D.H6$", names(data1))],
                                    n.rep=reps, normalize=FALSE, alpha=0.5)

mtor.h2.al <- glmBootstrapStability(data1[grep(".H2$", names(data1))],
                                    data1[grep(".H6$", names(data1))],
                                    n.rep=reps, normalize=FALSE, alpha=0.5)
...
table1 <- Reduce(function(x, y) {
  ...
  res$Row.names <- NULL
  return(res)
}, c(mtor.h2.si, list(mtor.h2.k1, mtor.h2.k2, mtor.h2.al)), accumulate=FALSE)
...
data2  <- unlist(getSingleCellData(wells), recursive=FALSE)
data2  <- lapply(data2, cleanData, "lower")
data2  <- makeFeatureCompatible(data2)
...
```{r, stability-rand, cache=TRUE, collapse=TRUE, warning=FALSE}
random.si <- lapply(barcodes, function(bc, data, n) {
